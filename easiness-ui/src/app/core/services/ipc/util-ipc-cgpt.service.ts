...
export class UtilService {
  ...
  buildActiveFilters(obj: any, keyNameMap: any): string {
    ...
    for (const key in keyNameMap) {
      ...
      if (
        value &&
        type == "string" &&
        Object.prototype.hasOwnProperty.call(keyNameMap, key)
      ) {
  ...
  deepEqual(x: object, y: object): boolean {
    if (x === y) {
      return true;
    } else if (
      typeof x == "object" &&
  ...
  toNgbDate(date: Date | string): NgbDate {
    if (!date) {
      return null;
    }
    date = new Date(date);
  ...
  async getSomeDataFromIPC(): Promise<any> {
